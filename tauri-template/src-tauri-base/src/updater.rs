...
//
// The shared-drive path defaults to the value of the `UPDATE_PATH` env var
// or a tool-specific path set at compile time.
// Override for dev/testing by setting `TOOL_UPDATE_PATH`.
//
// File logging (release builds only):
//   %LOCALAPPDATA%\<ProductName>\updater.log
...
/// Return the configured update path (env var override or panic).
///
/// Callers should set `UPDATE_PATH_ENV_VAR` or provide a hardcoded default.
...
pub fn check_for_update() -> UpdateCheckResult {
    ...
    let remote = match Version::parse(&latest.version) {
        Ok(v) => v,
        Err(e) => {
            log_updater(&format!("Invalid version in latest.json: {e}"));
            return UpdateCheckResult::UpToDate;
        }
    };
    ...
    if remote > current {
        log_updater(&format!("Update available: {current_str} → {}", latest.version));
...
pub fn copy_installer_with_progress(
    ...
    log_updater(&format!("Copy complete: {copied} bytes in {elapsed_ms} ms"));
    Ok(dest)
}
