...
```toml
desktop-toolkit = { git = "https://github.com/chamber-19/desktop-toolkit", tag = "v2.3.0" }
```
...
```json name=frontend/package.json
{
  ...
  "dependencies": {
    "@chamber-19/desktop-toolkit": "^2.3.0"
  },
  "devDependencies": {
    "@tauri-apps/cli": "^2",
    "vite": "^6"
...
> **Note:** `^2.3.0` is the current recommended pin. v2.2.5 is the first release where the
    ...
    steps:
      ...
      - name: Build desktop-toolkit-updater shim
        ...
        run: |
          ...
          $tag = "v2.3.0"   # Keep in sync with Cargo.toml and requirements.txt
...
#!/usr/bin/env node
// Usage: node scripts/bump-toolkit.mjs 2.3.0
...
Run: `node scripts/bump-toolkit.mjs 2.3.0`
...
| `NSIS error: command File not valid outside Section or Function` on line 113 of `hooks.nsh` | Consumer is using `desktop-toolkit` ≤ 2.2.4 | Bump to `^2.3.0` in `package.json`, re-run `npm install`, and rebuild |
...
- [ ] Added `@chamber-19/desktop-toolkit@^2.3.0` to `frontend/package.json` dependencies
...
- [ ] (If using Python) Added `chamber19-desktop-toolkit @ git+…@v2.3.0` to `backend/requirements.txt`
