...
.PARAMETER Tag
    The Git tag to publish, e.g. "v1.0.0".
...
if (-not (Test-Path $DrivePath)) {
    throw "Shared drive path not found: $DrivePath`nEnsure Google Drive for Desktop is running and the drive is mounted."
}
...
$TempDir = Join-Path $env:TEMP "r3p-publish-$Tag"
